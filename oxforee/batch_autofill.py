...
class BatchAutoFill:
    def __init__(self, input_file: str, output_file: str):
        ...
        self.translate_api = "https://api.mymemory.translated.net/get"
    ...
    async def fetch_thai_translation(self, session: aiohttp.ClientSession, word: str) -> Optional[str]:
        """Fetch Thai translation with clean up"""
        try:
            url = f"{self.translate_api}?q={word}&langpair=en|th"
            async with session.get(url) as response:
                if response.status == 200:
                    data = await response.json()
                    
                    if data and data.get('responseData'):
                        translate = data['responseData'].get('translatedText', '')
                        
                        # Validations
                        if not translate: return None
                        if translate.lower() == word.lower(): return None # No translation allowed
                        
                        # Clean up
                        # Sometimes API returns "word - translation", keep only translation
                        if "-" in translate:
                            parts = translate.split("-")
                            if len(parts) == 2 and parts[0].strip().lower() == word.lower():
                                translate = parts[1].strip()
